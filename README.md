...
Также нужно установить следующие переменные окружения любым удобным способом (например, в файле `.env`):
...
Если не указать все выше перечисленные поля, не считая необязательных, то прод не запустится. Если не указывать необязательные поля, то высветится предупреждение.
...
> УКАЗАНИЕ 1: Первый зарегистрировавшийся пользователь сайта автоматически получает роль MAIN_ADMIN.

> УКАЗАНИЕ 2: Управление от лица администрации происходит пока только через API. Управление пользователями администраторами пока не реализовано.
