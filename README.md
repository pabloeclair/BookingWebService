...
```
УКАЗАНИЕ: Первый зарегистрировавшийся пользователя сайта автоматически получает роль MAIN_ADMIN.
```
```
УКАЗАНИЕ: Управление администрацией происходит пока только через API. Изменение пользователей администраторами пока не реализовано.
```
